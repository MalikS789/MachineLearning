...
class NeuralNetwork():
    ...
    def forward_pass(self, inputs):
        ...
        for i in range(self.n_hidden):
            ...
            self.y_hidden[i] = sigmoid([hidden_neuron])[0]
        ...
        for i in range(self.n_out):
            ...
            self.y_out[i] = sigmoid([output_neuron])[0]
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        for k in range(self.n_out):
            output_deltas = (outputs[k] - targets) * sigmoid_derivative(inputs[k])
        ...
        for j in range(len(hidden_deltas)):
            ...
            for k in range(len(outputs)):
               hidden_neuron += outputs[k] * hidden_deltas[j]
            hidden_deltas[j] = sigmoid_derivative(outputs[k]) * hidden_neuron

        # Step 3. update the weights of the output layer
        for j in range(len(self.y_hidden)):
            for k in range(len(output_deltas)):
               self.w_out[j,k] -= hidden_deltas[j] * output_deltas[k] * learning_rate
        ...
        for j in range(len(inputs)):
            for k in range(len(hidden_deltas)):
               self.w_hidden[j,k] -= learning_rate * hidden_deltas[k] * inputs[j]
